...
function PaintObject(maincvs) {
  ...
  $("#fillColor").change(function () {
    mainContext.fillStyle = frontContext.fillStyle =
      "#" + $("#fillColor").val();
  });
  ...
  this.processPaintCommand = function (paintCommand) {
    ...
    switch (paintCommand.type) {
      case "pencilMove":
        ...
        mainContext.moveTo(
          paintCommand.previousMousePos.x,
          paintCommand.previousMousePos.y
        );
        mainContext.lineTo(
          paintCommand.currentMousePos.x,
          paintCommand.currentMousePos.y
        );
  ...
  this.sendOrderToDrawFrontCanvasOnMainCanvas = function () {
    ...
    socket.emit("paint", paintCommand); // send the order the server and it will send back to all tghe client
